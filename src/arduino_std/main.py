...
def main(com_port, midi_port, baud_rate, pad_config, output_msg_type):
    # object initiations
    serial = Serial(com_port=com_port, serial_rate=baud_rate)
    midi = Midi(virtual_port=midi_port, output_msg_type=output_msg_type)
    ...
    try:
        while True:
            ...
            if msg_recvd['cmd'] in ['control_change', 'program_change',
                ...
                midi.send_midi_msg(midi_msg, msg_recvd)
            ...
            # else it's a note_on/note_off msg
                ...
                for note in notes: 
                    midi_json = {
                        ...
                        'min_vel_out': note['min_vel_out'],
                        'max_vel_out': note['max_vel_out'],
                        'min_vel_in': note['min_vel_in']
                    }
                    ...
                    midi.store_midi_msg(midi_msg)
                    midi.send_midi_msg(midi_msg, midi_json)
