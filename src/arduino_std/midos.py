...
class Midi(object):
    ...
    def map_velocity(self, msg_json):
        ...
        if msg_json['velocity'] < msg_json['min_vel_in']:
            return 0
        else:
            return int((msg_json['velocity']-0) * (msg_json['max_vel_out']-msg_json['min_vel_out']) / 
                    (127-0) + msg_json['min_vel_out'])
    ...
    def send_midi_msg(self, midi_msg, msg_json, port=MIDI_PORT):
        ...
        elif self.output_msg_type == "json":
            msg_json["port"] = port
